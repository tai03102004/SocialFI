...
export class CronService {
  ...
  private async runDailyAIProcessing() {
    this.isRunning = true;
  ...
  private async resolveExpiredQuests() {
    try {
  ...
  private evaluateQuestResult(quest: any, participant: any): boolean {
    ...
    const hasValidPrediction =
      typeof participant.prediction === "string" &&
  ...
  private async awardQuestRewards(address: string, rewards: any) {
    try {
