...
export class GameFiController {
    ...
    async getPlayerPredictions(req, res) {
        try {
            const { address } = req.params;
    ...
    async getLeaderboard(req, res) {
        try {
            ...
            const leaderboard = await User.find()
                .sort({ [sortField]: -1 })
                .limit(Number(limit))
                .select("address username gameScore socialScore correctPredictions totalPredictions");
            res.json({
                success: true,
                data: leaderboard,
            });
    ...
    async updateProfile(req, res) {
        try {
            ...
            const user = await User.findOneAndUpdate({ address: address.toLowerCase() }, {
                username,
                email,
                profileImage,
                lastActive: new Date(),
...
//# sourceMappingURL=gamefi.controller.js.map